    ...
    >
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-white/50">Track your debts and monitor active loans.</p>
        </div>
        <div className="flex items-center gap-2 p-1 bg-white/5 rounded-2xl border border-white/10">
          <button
            ...
            className={cn(
              ...
              activeView === 'debts' ? "bg-white text-black" : "text-white/50 hover:text-white"
            )}
          >
            Debts
          </button>
          <button
            ...
            className={cn(
              "px-6 py-2 rounded-xl text-sm font-semibold transition-all",
              activeView === 'loans' ? "bg-white text-black" : "text-white/50 hover:text-white"
            )}
          >
            Loans
          </button>
          ...
          {activeView === 'debts' ? (
            <form onSubmit={handleAddDebt} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="space-y-2">
                <label className="text-sm font-medium text-white/50">Debt Name</label>
                <input
                  type="text"
                  required
                  value={debtForm.name}
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-white/50">Total Amount</label>
                <input
                  type="number"
                  required
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-white/50">Remaining</label>
                <input
                  type="number"
                  required
              ...
              <div className="flex items-end gap-3">
                <button type="submit" className="flex-1 bg-white text-black py-3 rounded-xl font-semibold">Save</button>
                <button type="button" onClick={() => setIsAdding(false)} className="px-4 py-3 rounded-xl border border-white/10">X</button>
              </div>
            </form>
          ) : (
            <form onSubmit={handleAddLoan} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="space-y-2">
                <label className="text-sm font-medium text-white/50">Loan Name</label>
                <input
                  type="text"
                  required
                  value={loanForm.name}
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-white/50">Principal</label>
                <input
                  type="number"
                  required
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-white/50">Interest Rate (%)</label>
                <input
                  type="number"
                  step="0.01"
                  required
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {activeView === 'debts' ? (
          debts.map(debt => (
            <div key={debt.id} className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-6 relative group">
              <button
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-white/50">Personal Debt</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-end">
                  <div className="space-y-1">
                    ...
                    <p className="text-3xl font-bold text-rose-400">{formatCurrency(debt.remainingAmount)}</p>
                  </div>
          ...
          loans.map(loan => (
            <div key={loan.id} className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-6 relative group">
              <button
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-white/50">Active Loan</p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-1">
                  <p className="text-xs text-white/40 uppercase font-bold tracking-widest">Principal</p>
                  <p className="text-xl font-bold">{formatCurrency(loan.principal)}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-xs text-white/40 uppercase font-bold tracking-widest">Interest</p>
                ...
                <div className="space-y-1">
                  <p className="text-xs text-white/40 uppercase font-bold tracking-widest">Term</p>
                  <p className="text-xl font-bold">{loan.termMonths} Months</p>
                </div>
                <div className="space-y-1">
                  <p className="text-xs text-white/40 uppercase font-bold tracking-widest">Started</p>
